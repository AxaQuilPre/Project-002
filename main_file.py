...
def patrol_calculator():
    ...
    question = int(input("Were at least 4 settlement cleared? \n Choices: \n Type 1 for yes \n Type 2 for no \n"))
    if question == 1:
        total += 50000
...
#Logistics:
    final_draft = final_draft + "\n \n **Logistics**"
    question = int(input("How many ground vehicles were shipped to Sochraina City? \n"))
    if question > 0: 
        total -= question * 1000
        final_draft = final_draft + f"\n {question} ground vehicles shipped to Sochraina: -${number_divider(question * 1000)}"
    question = int(input("\n How many UA ground vehicles were shipped to Sochraina City? \n"))
    if question > 0:
        total -= question * 5000
        final_draft = final_draft + f"\n {question} UA ground vehicles shipped to Sochraina: -${number_divider(question * 5000)}"
    question = int(input("How many air vehicles were shipped to Sochraina City? \n"))
    if question > 0: 
        total -= question * 5000
        final_draft = final_draft + f"\n {question} air vehicles shipped to Sochraina: -${number_divider(question * 5000)}"
    question = int(input("How many UA air vehicles were shipped to Sochraina City? \n"))
    if question > 0: 
        total -= question * 7000
        final_draft = final_draft + f"\n {question} UA Air vehicles shipped to Sochraina: -${number_divider(question * 7000)}"
    
    question = int(input("How many ground vehicles were shipped to UN? \n"))
    if question > 0: 
        total -= question * 2000
        final_draft = final_draft + f"\n {question} ground vehicles shipped to UN: -${number_divider(question * 2000)}"
    question = int(input("How many UA ground vehicles were shipped to UN? \n"))
    if question > 0:
        total -= question * 7000
        final_draft = final_draft + f"\n {question} UA ground vehicles shipped to UN: -${number_divider(question * 7000)}"
    question = int(input("How many air vehicles were shipped to UN? \n"))
    if question > 0: 
        total -= question * 7000
        final_draft = final_draft + f"\n {question} air vehicles shipped to UN: -${number_divider(question * 7000)}"
    question = int(input("How many UA air vehicles were shipped to UN? \n "))
    if question < 0: 
        total -= question * 10000
        final_draft = final_draft + f"\n {question} UA Air vehicles shipped to UN: -${number_divider(question * 10000)}"
...
def divisional_calculator():
    ...
    question = int(input("Were at a 4 settlement cleared? \n Choices: \n Type 1 for yes \n Type 2 for no \n"))
    if question == 1:
        total += 100000
...
#This is an automation for financial agents. This code belongs to AxaQuilPre. Any reproduction or inauthorized use is prohibited. 
